-- ══════════════════════════════════════════════════════════════════════════════
-- Migration 038: Fix fn_pl_summary — live approx_payout from commissions table
-- MediGlove ERP
--
-- Bug: fn_pl_summary (M034) reads approx_payout from monthly_payouts table.
-- monthly_payouts is only populated by Make.com month-end batch — empty during
-- the month → approx_payout = 0 even when invoices have been paid.
--
-- Fix: Read approx_payout directly from commissions table (status='Actual'),
-- filtered by the same (p_year, p_month) window via invoices.paid_at.
-- This is identical to what get_dashboard_kpis() reports — both KPIs are now
-- in sync at all times.
--
-- monthly_payouts table is preserved for Make.com batch settlement history.
-- ══════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  RETURN (
    WITH
    -- ── All paid invoice line items for the period ──────────────────────────
    paid_items AS (
      SELECT
        ii.product_id,
        ii.qty,
    ...
    -- ── Approx payout: live sum from commissions table ───────────────────────
    -- Reads Actual commission rows whose linked invoice was paid this period.
    -- Stays in sync with get_dashboard_kpis() actual_commission at all times.
    -- No longer depends on monthly_payouts being populated by Make.com batch.
    total_payout AS (
      SELECT COALESCE(SUM(c.amount), 0) AS approx_payout
      FROM commissions c
