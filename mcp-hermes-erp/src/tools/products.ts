...
export function registerProductTools(server: McpServer) {
  ...
  server.tool(
    "erp_list_products",
    "List products in the MediGlove ERP catalogue. Supports search by name/SKU and optional category filter. Returns price, unit, stock quantity, and active status.",
    {
      ...
      active_only: z.boolean().default(true).describe("If true, exclude discontinued/inactive products"),
      limit:    z.number().int().min(1).max(200).default(50),
      offset:   z.number().int().min(0).default(0),
    },
    async ({ search, category, active_only, limit, offset }) => {
      let q = supabase
        .from("products")
        .select(
          "id, name, sku, category, unit_price, unit, stock_qty, is_active, created_at",
          { count: "exact" }
        )
        .order("name")
  ...
  server.tool(
    ...
    async ({ id, sku }) => {
      ...
      let q = supabase
        .from("products")
        .select("id, name, sku, category, unit_price, unit, stock_qty, is_active, description, created_at");
