...
export function registerClientTools(server: McpServer) {
  ...
  server.tool(
    ...
    "Create a new client. Performs a find-or-create: if a client with the same name already exists for the given owner, returns the existing record instead of creating a duplicate.",
    {
      ...
      contact_address:z.string().optional().describe("Business address"),
      industry:       z.string().optional().describe("Industry / business type"),
    },
    async ({ name, region, owner_id, contact_person, contact_phone, contact_email, contact_address, industry }) => {
      ...
      const payload: Record<string, unknown> = { name: name.trim(), region, owner_id };
      ...
      if (contact_address) payload.contact_address = contact_address;
